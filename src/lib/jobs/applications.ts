...
export type AppliedJobItem = Pick<
  Tables<"job_applications">,
  "created_at" | "id" | "status" | "viewed_at"
> & {
  job: Pick<
    Tables<"jobs">,
    | "contract_type"
...
export async function getAppliedJobsForCurrentUser(
  supabase: SupabaseClient<Database>,
  profileId: string,
) {
  const { data, error } = await supabase
    .from("job_applications")
    .select(
      "id, status, viewed_at, created_at, jobs!inner(id, title, location, work_mode, contract_type, skills, tools, welding_processes, materials, required_certifications, published_at, created_at, companies(id, name, location, logo_url))",
    )
    .eq("applicant_profile_id", profileId)
    .order("created_at", { ascending: false });
