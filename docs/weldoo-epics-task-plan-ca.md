# Weldoo - Planificacio per epiques i tasques

## Com utilitzar aquest document

Aquest document reorganitza el desenvolupament de Weldoo per epiques de producte. Una epica representa una area gran de la plataforma, com Feed, Login, Missatges o Jobs. Una tasca representa una unitat executable i validable, com Crear post, Guardar job o Crear missatge.

Recomanacio de treball:

1. Treballar una epica cada vegada, excepte correccions petites.
2. Executar tasques petites amb Codex, no epiques senceres.
3. Validar cada tasca amb usuari real, base de dades i navegador.
4. Separar sempre funcionalitat, dades i ajust visual quan el risc sigui alt.
5. Abans de passar a una nova epica, deixar documentades les validacions pendents.

Prompt base per a Codex:
...
- `Done`: implementat i validat de forma basica.
- `Needs polish`: funcional, pero pendent d'ajust visual o UX.
- `Planned`: pendent d'implementar.
- `Phase 2`: fora del primer MVP funcional.

## Epic 0 - Foundation, Architecture and Deployment

Objectiu: mantenir una base tecnica estable per poder desenvolupar sprint a sprint sense acumular deute critic.
...
Objectiu: permetre que un usuari es registri, entri, recuperi password i completi el seu perfil inicial.

### Task 1.1 - Sign in

Estat: `Done / Needs visual QA`
...
- Aquesta importacio ha de ser via API/OAuth oficial, no scraping.
- Amb LinkedIn OpenID Connect normalment podem esperar dades basiques d'identitat amb scopes com `openid`, `profile` i `email`.
- Camps com experiencia laboral completa, skills, certificacions o historial professional no s'han d'assumir disponibles per defecte.
- La dada importada ha de quedar com a proposta editable abans de completar onboarding.
...
- Aquesta tasca es diferent de l'alta amb LinkedIn: l'usuari ja existeix a Weldoo.
- La importacio ha de requerir consentiment explicit abans d'actualitzar dades.
- Si Supabase no permet enllaçar identitats de forma suficient per aquest cas, cal plantejar un flux OAuth propi del servidor per importar dades sense canviar el metode principal de login.
...
Objectiu: fer que la navegacio principal sigui coherent amb el prototip en desktop i mobile.

### Task 2.1 - Header and main navigation

Estat: `Done / Needs polish`
...
Objectiu: crear el nucli social de Weldoo: publicar, veure, comentar, reaccionar, guardar i moderar contingut.

### Task 3.1 - Feed layout and data loading

Estat: `Done / Needs visual polish`
...
Estat: `Done / Needs prototype parity`
...
Objectiu: donar valor al perfil professional, empresa i centre de formacio com a identitat publica dins la xarxa.

### Task 4.1 - Professional profile edit

Estat: `Done / Needs advanced polish`
...
Objectiu: descobrir perfils i crear relacions professionals.
...
Objectiu: permetre comunicacio basica entre usuaris sense construir encara un xat complex.
...
Objectiu: informar l'usuari d'activitat rellevant sense muntar encara una infraestructura complexa.
...
Objectiu: permetre descobrir, guardar, aplicar i gestionar ofertes de feina.
...
Objectiu: permetre descobrir cursos, webinars i formacions, i registrar interes.
...
Objectiu: permetre descobrir events del sector i registrar interes o assistencia.
...
Objectiu: donar control basic sobre compte, privacitat i preferencies.
...
Objectiu: centralitzar l'activitat de l'usuari: jobs, events, cursos i progres.
...
Objectiu: permetre operar la plataforma i controlar qualitat del contingut.
...
Objectiu: incorporar IA de forma pragmatica, util i no critica per al MVP inicial.
...
Objectiu: preparar Weldoo per operar de forma fiable, primer com a MVP en Vercel + Supabase i, mes endavant, amb una arquitectura capaç de suportar creixement internacional i carrega alta.
